...
df_all["timestamp"] = pd.to_datetime(df_all["Timestamp"], unit="s", errors="coerce")
...
print(f"Gesamter Stromverbrauch seit {available_months_str[choice_idx]}: {total_consumption_selected:.2f} kWh")
...
scaling_factor = total_consumption_selected / total_consumption_all if total_consumption_all > 0 else 1
total_by_time_all_normalized = total_by_time_all * scaling_factor

plt.figure(figsize=(16,6))
plt.bar(total_by_time_selected.index, total_by_time_selected.values, color="skyblue", edgecolor="black", label="Ausgewählter Zeitraum")
plt.plot(total_by_time_all_normalized.index, total_by_time_all_normalized.values, color="red", linestyle="--", linewidth=2, alpha=0.7, label="Gesamtes Jahr (normalisiert)")

plt.xlabel("Zeit (hh:mm)")
plt.ylabel("Gesamter Verbrauch (kWh)")
plt.title(f"Vergleich Stromverbrauch pro Zeit-Slot\nSeit {available_months_str[choice_idx]}: {total_consumption_selected:.2f} kWh, Gesamt: {total_consumption_all:.2f} kWh")
...
bars = plt.bar(["Teure Stunden", "Günstige Stunden"], [expensive_consumption, cheap_consumption], color=["orange", "lightgreen"])
...
for i, v, pct in zip([0,1], [expensive_consumption, cheap_consumption], [expensive_pct, cheap_pct]):
    plt.text(i, v + max(cheap_consumption, expensive_consumption)*0.05 + 1, f"{v:.2f} kWh\n({pct:.1f}%)", ha="center", fontweight="bold")
plt.ylim(0, max(cheap_consumption, expensive_consumption)*1.25)  # mehr Platz nach oben
plt.tight_layout()
plt.show()
